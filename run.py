...
def main():
    ...
    # FROZEN MODE: reviews disabled so the strategy never mutates. It will
    # accumulate a full sample so you can judge v12's real performance.
    FROZEN = True
    stale_h = STALE_REVIEW_H.get(st["tf"], 48)
    hours_since = (now_ms() - st.get("last_review_t", 0)) / 3600_000
    due = (not FROZEN) and (st["trades_since_review"] >= REVIEW_EVERY or hours_since > stale_h)
    if due:
        # Don't fix what's working: if this version has traded a real sample
        # and is clearly profitable, let it keep running instead of replacing
        # it. Compute stats for THIS version's trades since the last review.
        cur_ver = st["strategy"]["version"]
        vtrades = [t for t in st["trades"] if t["strategy_version"] == cur_ver]
        keep = False
        if len(vtrades) >= REVIEW_EVERY:
            wins = [t for t in vtrades if t["pnl"] > 0]
            losses = [t for t in vtrades if t["pnl"] <= 0]
            ...
            if net > 0 and pf >= 1.1:
                keep = True
                st["trades_since_review"] = 0  # reset counter, review later
                st["last_review_t"] = now_ms()
                log(st, f"Keeping v{cur_ver}: profitable over "
                        f"{len(vtrades)} trades (PF {pf:.2f}, net "
                        f"${net:.2f}) - not revising.", "ai")
        if not keep:
