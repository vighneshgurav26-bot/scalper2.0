...
FALLBACK = sanitize({
    ...
    "timeframe": "5m",
    "markets": ["BTC"],
    "rationale": "Reconstruction of v12 from its ledger review: 5m BTC breakout, TP 0.55%/SL 0.22%, 0.15% trailing stop. Frozen for a full-sample test - NOT expected to be profitable (v12 self-reported PF 0.69).",
    "longConditions": [
        {"indicator": "breakout", "period": 15, "direction": "high"},
    ],
    "shortConditions": [
        {"indicator": "breakout", "period": 15, "direction": "low"},
    ],
...
def bootstrap(st):
    # FROZEN MODE: deploy the hardcoded v12-approx strategy, no tournament.
    strat = dict(FALLBACK)
    st["strategy"] = strat
    st["tf"] = strat["timeframe"]
    st["history"] = [{"version": 1, "t": now_ms(),
                      "analysis": strat["rationale"],
                      "changes": ["FROZEN v12-approx: 5m BTC breakout, "
                                  "TP0.55/SL0.22, trail0.15 - full-sample test"],
                      "strategy": strat}]
    st["last_review_t"] = now_ms()
    log(st, f'FROZEN strategy online: "{strat["name"]}" | '
